...
async function fetchProducts(category = null) {
    try {
        const url = category 
            ? `${window.App.routes.productsGet}?category=${encodeURIComponent(category)}`
            : window.App.routes.productsGet;
...
function renderCategoryButtons() {
    ...
    const categories = [...new Set(productsCache.map(p => p.category).filter(Boolean))].sort();
    
    buttonContainer.innerHTML = `
        <button class="px-3 py-2 rounded text-sm ${!currentCategory ? 'bg-indigo-600 text-white' : 'bg-white border'}" data-category="">
            All
        </button>
    `;
    ...
    categories.forEach(cat => {
        ...
        btn.className = `px-3 py-2 rounded text-sm ${currentCategory === cat ? 'bg-indigo-600 text-white' : 'bg-white border'}`;
        btn.textContent = cat;
